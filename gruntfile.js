/*
 * Lym gruntfile. Don't modify this file unless you want to change the way Lym behaves.
 *
 ...
 * init : Forces a bower install on all Lym components with a "bower.json" file in their
 ...
 *          Lym.
 *
 * Additional commands : --lym JSON, where JSON is a JSON string of LymConfig overrides.
 *
 * Lym's configuration is stored in local lym.json file. This file can be overriden
 * by passing in the same JSON stucture with --lym paramater, or by placing a lym.json
 * in the parent folder to this folder. If both are used the --lym parameter option wins.
 * */
...
module.exports = function(grunt) {
    ...
    try{
        ...
        grunt.log.writeln('Using lym from command line.');
    } catch(ex){
    ...
    var gruntConfig = {
        ...
        // Compiles sass to css. Sass is built by lym-build-master-sass task.
        compass: {
            default: {
                options: {
                    sassDir: [path.join(config.lymConfig.tempFolder, 'scss')],
                    cssDir : path.join(targetBuildFolder, config.lymConfig.cssFolder)
                }
            }
        },
    ...
    grunt.registerTask('init', ['lym-bower-update']);
